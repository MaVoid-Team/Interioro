 ...
 * Hardcoded for Saudi Arabia (SA) in address forms.
 */
...
/**
 * Get the list of Saudi Arabia regions for display purposes
 */
export function getSaudiRegions() {
    const saudiArabia = (allCountries as unknown as CountryTuple[]).find(
        (c) => c[1] === "SA"
    )
    if (!saudiArabia) return []
    return saudiArabia[2].map(([name, code]) => ({ name, shortCode: code }))
}
